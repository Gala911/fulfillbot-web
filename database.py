...
# Возможные статусы сотрудничества с компанией. Ключ хранится в базе, значение — подпись на бейдже.
PARTNER_STATUSES = {
    "working": "Работаем",
    "signing": "На подписании",
    "worked": "Работали раньше",
    "rejected": "Не подошли",
}


def update_company_details(company_id, website=None, city=None, notes=None):
    with closing(get_conn()) as conn, conn:
        if notes is not None:
            conn.execute(
                "UPDATE companies SET website = ?, city = ?, notes = ? WHERE id = ?",
                (website, city, notes, company_id),
            )
        else:
            # notes не передан явно — оставляем как было (например, при
            # редактировании сайта/города пользователем через интерфейс,
            # где заметка не участвует в форме)
            conn.execute(
...
def set_partner_status(company_id, status):
    if status and status not in PARTNER_STATUSES:
        raise ValueError("Unknown partner status")
    with closing(get_conn()) as conn, conn:
        conn.execute(
            "UPDATE companies SET partner_status = ? WHERE id = ?",
            (status or None, company_id),
        )


def set_review(company_id, rating=None, comment=None):
    """Личная оценка (1-5) и комментарий пользователя по компании."""
    if rating is not None and rating not in (1, 2, 3, 4, 5):
        raise ValueError("Rating must be 1-5")
    with closing(get_conn()) as conn, conn:
        conn.execute(
            "UPDATE companies SET user_rating = ?, user_comment = ? WHERE id = ?",
...
def get_tariffs_for_company(company_id):
    with closing(get_conn()) as conn:
        return conn.execute(
            "SELECT * FROM tariffs WHERE company_id = ? AND price IS NOT NULL", (company_id,)
        ).fetchall()
